...
def get_hyperedge_index(dataset, args):
    for i in range(len(dataset)):
        ...
        adj_raw = to_scipy_sparse_matrix(edge_index, num_nodes=num_nodes)                           

        hyper_adj = (adj_raw.dot(adj_raw)).multiply(adj_raw)                                        

        hyperedge_all = []
        for i in range(hyper_adj.shape[0]):
            start_idx = hyper_adj.indptr[i]
            end_idx = hyper_adj.indptr[i + 1]
            row_nonzero_columns = hyper_adj.indices[start_idx:end_idx].tolist()                    
            row_nonzero_columns.append(i)                                                           
            hyperedge_all.append(tuple(row_nonzero_columns))                                          

        hyperedge_unique = setDuplicate(hyperedge_all)                                               

        nonzero_rows, nonzero_cols = adj_raw.nonzero()                                              
        for i in range(len(nonzero_rows)):
            head_node = nonzero_rows[i]
            tail_node = nonzero_cols[i]

            is_in_tuple = False
            for s in hyperedge_unique:
                if head_node in s and tail_node in s:
                    is_in_tuple = True
                    break
            
            if not is_in_tuple:
                hyperedge_unique.append((head_node, tail_node))

        hyperedge_unique=setDuplicate(hyperedge_unique)                                        

        index_node = []
        index_hyperedge = []

        for i, s in enumerate(hyperedge_unique):
            index_node.extend(list(s))
            index_hyperedge.extend([i] * len(s))

        index_node = torch.tensor(index_node, device = device)
        index_hyperedge = torch.tensor(index_hyperedge, device = device)

        hyperedge_index = torch.cat((index_node.unsqueeze(0), index_hyperedge.unsqueeze(0)), dim=0)
...
def get_ad_dataset_TU(args, split, need_str_enc=True, hypergraph_constr=True):     
    DS = args.DS
    path = osp.join(osp.dirname(osp.realpath(__file__)), '.', 'data', DS)
    ...
    if hypergraph_constr:
        data_list = get_hyperedge_index(data_list, args)
...
def get_ad_dataset_Tox21(args, need_str_enc=True, hypergraph_constr=True):
    ...
    if hypergraph_constr:
        data_train = get_hyperedge_index(data_train, args)
        data_test = get_hyperedge_index(data_test, args)
